...
def _save_preview_figure(image, title, save_path, cmap='gray'):
...
def _save_merged_preview(original_image, gt_image, anomaly_image, save_path):
    fig = plt.figure(figsize=(15, 5))
    panels = [
        (original_image, 'Original image'),
        (gt_image, 'Ground truth'),
        (anomaly_image, 'HSI anomaly'),
    ]
    for idx, (image, title) in enumerate(panels, start=1):
...
def HAD(args):
    ...
    original_result = _normalize_for_display(np.mean(args.HSI_3D, axis=2))
    gt_result = _normalize_for_display(gt)
    anomaly_result = _normalize_for_display(AD_result)
    ...
    _save_merged_preview(original_result, gt_result, anomaly_result, os.path.join(args.save_dir, args.dataset_name + '_overview.png'))

    S_save = dict()
    S_save['original'] = original_result
    S_save['gt'] = gt
    S_save['det'] = AD_result
    S_save['auc_pd_pf'] = PD_PF_auc
    S_save['auc_pf_tau'] = PF_tau_auc
    result_file_name = args.dataset_name + str('.mat')
    sio.savemat(os.path.join(args.save_dir, result_file_name), S_save)

    metrics_file = os.path.join(args.save_dir, args.dataset_name + '_metrics.txt')
    with open(metrics_file, 'w', encoding='utf-8') as f:
        f.write('Dataset: %s\n' % args.dataset_name)
        f.write('AUC PD_PF_auc: %.5f\n' % PD_PF_auc)
        f.write('AUC PF_tau_auc: %.5f\n' % PF_tau_auc)


if __name__ == '__main__':

    parser = argparse.ArgumentParser()
    parser.add_argument('--data_dir', type=str, default="./Data/abu-beach-4.mat")
